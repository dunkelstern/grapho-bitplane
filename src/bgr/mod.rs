...
//!
//! In memory representation is (one byte each): B, G, R, B, G, R, ...
...
impl PixelBuffer for BGRPixelBuffer {
    ...
    fn new_with_background(width: usize, height: usize, stride: Option<usize>, color: Self::ColorType) -> Self {
        let mut representation: [u8; 3] = color.into();
        let line_width = stride.unwrap_or(width * 3);
        let data:Vec<u8>;
    ...
    fn get_pixel(&self, x: usize, y: usize) -> Result<Self::ColorType, PixelBufferError> {
        ...
        let start = x * 3 + y * self.stride;
        let slice = [self.data[start + 2], self.data[start + 1], self.data[start + 0]];
...
mod tests {
    ...
    #[test]
    fn empty_bgr_buffer() {
        let buffer = BGRPixelBuffer::new(2, 2, None);
    ...
    #[test]
    fn prefilled_bgr_buffer() {
        let data = vec![0, 255, 64, 0, 255, 64, 0, 255, 64, 0, 255, 64];
        let copy = data.clone();
        let buffer = BGRPixelBuffer::new_with_data(2, 2, None, data).unwrap();
        assert_eq!(buffer.data.len(), 12);
    ...
    #[test]
    fn bg_bgr_buffer() {
        let buffer = BGRPixelBuffer::new_with_background(
            2, 2, None,
    ...
    fn set_pixel_rgb_buffer() {
        let mut buffer = BGRPixelBuffer::new(2, 2, None);
    ...
    fn get_pixel_rgb_buffer() {
        ...
        let buffer = BGRPixelBuffer::new_with_data(2, 2, Some(8), data).unwrap();

        match buffer.get_pixel(0, 0) {
            Err(_error) => assert!(false),
            Ok(color) => assert_eq!(color, DigitalRGBColor{ r: 2, g: 1, b: 0 })
        }

        match buffer.get_pixel(1, 1) {
            Err(_error) => assert!(false),
            Ok(color) => assert_eq!(color, DigitalRGBColor{ r: 11, g: 10, b: 9 })
        }
